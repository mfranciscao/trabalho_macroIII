...
($C_t$): $\displaystyle \beta^tC_t^{-\sigma} = \beta^t \lambda_t P_t \ \ \Rightarrow \ \ C_t^{-\sigma} = \lambda_t P_t$

($N_t$): $\displaystyle \beta^tN_t^{\varphi} = \beta^t \lambda_t W_t \ \ \Rightarrow \ \ N_t^{\varphi} = \lambda_t P_t$
...
Dividing ($N_t$) FOC by ($C_t$) FOC, we have the standard equation of intratemporal substitution between consumption and leisure $\displaystyle C_t^{\sigma}N_t^{\varphi} = \frac{W_t}{P_t}$
...
Assuming that the law of one price is valid in all times (the same goods produced in different countries have the same price when converting to the domestic currency, using the nominal interest rate), we have $P_{i,t}^i(j) = \mathcal{E}_{i,t}P_{i,t}^i(j)$ for all $i, j \in [0,1]$.
...
## 9 - Uncovered interest parity and the terms of trade
