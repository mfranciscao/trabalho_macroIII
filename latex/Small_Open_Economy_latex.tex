...
\subsection{3 - Finding the optimal share between the domestic and
imported
...
\subsection{4 - Standard problem of the representative
...
(\(D_{t+1}\)):
\(\displaystyle \beta^t \lambda_t \mathbb{E} [Q_{t,t+1}] = \beta^{t+1} \mathbb {E}[\lambda_{t+1}] \ \ \Rightarrow \ \ \frac{\mathbb{E}[\lambda_{t+1}]}{\lambda_t} = \frac{\mathbb{E} [Q_{t,t+1}]}{\beta}\)
...
as \(\mathbb{E}[Q_{t,t+1}]\), is the price of a a riskless one-period
discount bond in domestic currency with gross return \(R_t\).

\hypertarget{log-linearization}{%
\subsection{5 - Log linearization}\label{log-linearization}}
